...
def get_branding(request):
    ...
    serializer = BrandingSerializer(core_settings, context={"request": request})
    return Response(serializer.data)


class BrandingLogoUpload(APIView):
    permission_classes = [IsAuthenticated, CoreSettingsPerms]
    parser_classes = [MultiPartParser, FormParser]

    def post(self, request):
        field = request.data.get("field", "brand_logo")
        if field not in ("brand_logo", "brand_favicon"):
            return notify_error("Invalid field")

        file = request.FILES.get("file")
        if not file:
            return notify_error("No file uploaded")

        if file.size > 2 * 1024 * 1024:
            return notify_error("File size must be under 2MB")

        core = CoreSettings.objects.first()
        old_file = getattr(core, field)
        if old_file:
            old_file.delete(save=False)

        setattr(core, field, file)
        core.save()
        return Response("ok")

    def delete(self, request):
        field = request.query_params.get("field", "brand_logo")
        if field not in ("brand_logo", "brand_favicon"):
            return notify_error("Invalid field")
    ...
    return Response(
        {
            ...
            "brand_name": core_settings.brand_name,
            "brand_primary_color": core_settings.brand_primary_color,
            "brand_secondary_color": core_settings.brand_secondary_color,
            "brand_accent_color": core_settings.brand_accent_color,
            "brand_header_color": core_settings.brand_header_color,
        }
    )
